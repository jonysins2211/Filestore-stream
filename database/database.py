...
class Rohit:
    ...
    def __init__(self, DB_URI, DB_NAME):
        ...
        self.masked_links = self.database['masked_links']       # shortener link masking
        self.fp_tokens = self.database['fp_tokens']             # fingerprint tokens
        self.hash_settings = self.database['hash_settings']     # selected algorithm
...
# Get current mode of a channel
    ...
    async def get_hash_algorithm(self) -> str:
        doc = await self.hash_settings.find_one({'_id': 'algorithm'})
    ...
    async def store_fp_token(self, token: str, hash_id: str, expires: float):
        import time
        await self.fp_tokens.update_one(
            {'_id': token},
            {'$set': {'hash_id': hash_id, 'expires': expires, 'created_at': time.time()}},
            upsert=True
        )

    async def validate_fp_token(self, token: str, hash_id: str) -> bool:
        import time
        doc = await self.fp_tokens.find_one({'_id': token})
        if not doc:
            return False
        if doc.get('expires', 0) < time.time():
            await self.fp_tokens.delete_one({'_id': token})
            return False
