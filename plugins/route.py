...
# Adapted from DP-FILE-STORE-BOT (TechVJ/VJBots)
# These routes serve the online watch page and direct download for files
...
TEMPLATE_DIR = _os.path.join(_os.path.dirname(_os.path.dirname(__file__)), "template")


def _load_template(name: str) -> str:
    path = _os.path.join(TEMPLATE_DIR, name)
    try:
...
async def watch_handler(request: web.Request):
    ...
    template = _load_template("stream.html")
    if not template:
        raise web.HTTPInternalServerError(text="Template not found")

    html = template % (
        file_name,          # <title>
        file_name,          # file title shown in page
...
@routes.get("/stream/{msg_id}", allow_head=True)
@routes.get("/dl/{msg_id}", allow_head=True)
async def stream_download_handler(request: web.Request):
    ...
    if not message or message.empty:
        raise web.HTTPNotFound(text="Message not found in channel")
    ...
    if not media:
        raise web.HTTPNotFound(text="No media in message")

    file_size = getattr(media, "file_size", 0)
    media_mime = getattr(media, "mime_type", "application/octet-stream") or "application/octet-stream"
    media_name = file_name or getattr(media, "file_name", None) or f"file_{msg_id}"

    # Determine if streaming (watch route) or download
